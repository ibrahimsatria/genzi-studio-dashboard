...
export function SignUpForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setInfo(
        "Check your inbox to confirm your email. After confirming, sign in to continue.",
      );
      router.refresh();
    } catch (e) {
  ...
  return (
    <form onSubmit={onSubmit} className="mt-8 flex flex-col gap-3">
      ...
      {info && (
        <p className="rounded-lg border border-green/40 bg-green/10 px-3 py-2 text-xs text-green">
          {info}
        </p>
      )}
